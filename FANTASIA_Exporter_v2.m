...
global TP
    ...
    for i = 1:length(TP.EX.Dir.SesFileIndex) 
        ...
        TP.EX.Dir.SesTrlNames{i} = TP.EX.Dir.MatNames(TP.EX.Dir.SesFileIndex(i)+1:...
            TP.EX.Dir.SesFileIndex(i+1)-1);
        end
    end
    ...
    for i = 1:length(endIndex)
        fileliststring = insertAfter(fileliststring, endIndex(i)+shift, '|');
        shift = shift + 1;
    end
...
%     TP.EX.Dir.FileNum =     length(TP.EX.Dir.FileListT);
%     fileliststring = [TP.EX.Dir.FileListT.name];
%     [~,endIndex] = regexp(fileliststring,'rec');
%     shift = 0;
%     for i = 1:length(endIndex)
%         fileliststring = insertAfter(fileliststring, endIndex(i)+shift, '|');
%         shift = shift + 1;
...
%                 TP.EX.D.File(i,1).SettingsMsg = [TP.EX.D.File(i,1).SettingsMsg ttemp];      
            ...
            display(['ERROR: ', num2str(i), 'th file is not a Session or Trial file'])
        end
       % ==================================================
